...
var fold, ref$, nameFromPath, stripString, util2, Node, Negatable, Block, Atom, Literal, Var, Key, Index, Slice, Chain, Call, List, Obj, Prop, Arr, Unary, Binary, Assign, Import, In, Existence, Fun, Typed, Class, Super, Parens, Splat, Jump, Throw, Return, While, For, Try, Switch, Case, If, Label, Cascade, JS, Require, Util, Vars, DECLS, UTILS, LEVEL_TOP, LEVEL_PAREN, LEVEL_LIST, LEVEL_COND, LEVEL_OP, LEVEL_CALL, PREC, TAB, ID, SIMPLENUM, slice$ = [].slice, toString$ = {}.toString;
...
exports.Fun = Fun = (function(superclass){
  ...
  prototype.extractTypes = function(parameter, params, chain){
    var types, i$, ref$, len$, j, item;
    types = [];
    if (parameter instanceof Typed) {
      if (parameter.expression.name != null) {
        chain[chain.length - 1] = parameter.expression.name;
      }
    ...
    if (parameter.items != null) {
      for (i$ = 0, len$ = (ref$ = parameter.items).length; i$ < len$; ++i$) {
        j = i$;
        item = ref$[i$];
        types = types.concat(this.extractTypes(item, params, chain.concat(j)));
      }
    }
    return types;
  };
  prototype.collapseTypes = function(parameter){
    ...
    if (parameter.items != null) {
      for (i$ = 0, len$ = (ref$ = parameter.items).length; i$ < len$; ++i$) {
        i = i$;
        item = ref$[i$];
        this.collapseTypes(parameter.items[i] = this.collapseTypes(item));
      }
    }
    return parameter;
  };
  prototype.compileParams = function(o, scope){
    var params, length, body, i$, p, types, len$, i, splace, rest, that, names, assigns, dic, vr, df, unaries, hasUnary, v, ref$, ref1$, typechecks, res$, type;
    ...
    for (i$ = 0, len$ = params.length; i$ < len$; ++i$) {
      i = i$;
      p = params[i$];
      types = types.concat(this.extractTypes(p, params, [i]));
      params[i] = this.collapseTypes(p);
    }
    for (i$ = 0, len$ = params.length; i$ < len$; ++i$) {
      i = i$;
      p = params[i$];
    ...
    for (i$ = 0, len$ = types.length; i$ < len$; ++i$) {
      type = types[i$];
      res$.push(type.generateCheck(o, scope, names));
    }
    typechecks = res$;
    if (typechecks.length) {
      (ref$ = this.body).prepend.apply(ref$, typechecks);
    }
    if (assigns.length) {
...
exports.Typed = Typed = (function(superclass){
  var prototype = extend$((import$(Typed, superclass).displayName = 'Typed', Typed), superclass).prototype, constructor = Typed;
  function Typed(types, expression, chain){
    var this$ = this instanceof ctor$ ? this : new ctor$;
    this$.types = types;
    ...
    this$.chain = chain != null
      ? chain
      : [];
    this$.stringyTypes = this$.types.map(function(it){
      return it.value;
    });
    return this$;
  } function ctor$(){} ctor$.prototype = prototype;
  prototype.reservedTypes = ['Undefined', 'Null', 'Any'];
  prototype.generateCheck = function(o, scope, refs){
    var stringLiteral, identifier, i$, ref$, len$, index, i, type;
    stringLiteral = function(value){
      return Literal("'" + value + "'");
    };
    identifier = Chain(Var(refs[this.chain[0]]));
    for (i$ = 0, len$ = (ref$ = slice$.call(this.chain, 1, -1 + 1 || 9e9)).length; i$ < len$; ++i$) {
      index = ref$[i$];
      identifier.add(Index((toString$.call(index).slice(8, -1) === 'Number' ? Literal : Key)(index)));
    }
    for (i$ = 0, len$ = (ref$ = this.stringyTypes).length; i$ < len$; ++i$) {
      i = i$;
      type = ref$[i$];
      if (in$(type, this.reservedTypes)) {
        scope.declare(type);
      }
    }
    return Chain(Var(util('typecheck'))).add(Call([identifier, Arr(this.types), Arr(this.stringyTypes.map(stringLiteral)), Arr(this.chain.map(stringLiteral))]));
  };
  return Typed;
}(Node));
