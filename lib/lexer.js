...
function addTypes(tokens){
  ...
  while (token = tokens[++i]) {
    ...
    if (seenParam && token[0] === 'ID') {
      if (((ref$ = tokens[i + 1][0]) === 'ID' || ref$ === 'CASE') || ((ref$ = tokens[i + 1][1]) === '[' || ref$ === '{') || (tokens[i + 1][1] === '?' && (tokens[i + 2][0] === 'ID' || ((ref$ = tokens[i + 2][1]) === '[' || ref$ === '{')))) {
        if ((tokens[i - 1][0] === 'CASE' && tokens[i - 2][0] === 'TYPE') || tokens[i - 1][0] === 'PARAM(' || ((ref$ = tokens[i - 1][1]) === ',' || ref$ === '[' || ref$ === '{')) {
          token[0] = 'TYPE';
        }
      }
    }
