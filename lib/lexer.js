...
function addImplicitParentheses(tokens){
  var i, brackets, seenParam, token, endi, ref$, tpair, tag, prev, ref1$, skipBlock, seenSwitch;
  i = 0;
  brackets = [];
  seenParam = false;
  while (token = tokens[++i]) {
    if (tokens[i - 1][0] === 'PARAM(') {
      seenParam = true;
    }
    if (tokens[i - 1][0] === ')PARAM') {
      seenParam = false;
    ...
    if (!(((ref1$ = prev[0]) === 'FUNCTION' || ref1$ === 'GENERATOR' || ref1$ === 'LET' || ref1$ === 'WHERE') || prev.spaced && able(tokens, i, true) && !seenParam)) {
      continue;
    }
    if (token.doblock) {
      token[0] = 'CALL(';
...
function detectTypes(tokens){
  var i, seenParam, token, ref$, prev2, prev, next, next2;
  i = 0;
  seenParam = false;
  while (token = tokens[++i]) {
    if (tokens[i - 1][0] === 'PARAM(') {
    ...
    if (seenParam && token[0] === 'ID') {
      ref$ = [tokens[i - 2], tokens[i - 1], tokens[i + 1], tokens[i + 2]], prev2 = ref$[0], prev = ref$[1], next = ref$[2], next2 = ref$[3];
      if (((ref$ = next[0]) === 'ID' || ref$ === 'CASE') || ((ref$ = next[1]) === '[' || ref$ === '{') || (next[1] === '?' && (next2[0] === 'ID' || ((ref$ = next[1]) === '[' || ref$ === '{')))) {
        if ((prev[0] === 'CASE' && prev2[0] === 'TYPE') || prev[0] === 'PARAM(' || ((ref$ = prev[1]) === ',' || ref$ === '[' || ref$ === '{')) {
          token[0] = 'TYPE';
        }
      }
    }
